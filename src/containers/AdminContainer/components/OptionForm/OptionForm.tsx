...
const OptionForm = ({ onSubmit, option }: OptionFormProps) => {
  ...
  useEffect(() => {
    if (option) {
      setValues({
        ...option,
        parent: option.parent.map((parentList) =>
          parentList.map((parent) => ({ label: parent.title, value: parent.id }))),
  ...
  return (
    <form onSubmit={handleSubmit} className={classes.wrapper}>
      <div className={classes.container}>
        ...
        <AdminAutocomplete
          list={useQuestions}
          handleOptions={(question) => ({ label: question.title, value: question.id })}
